      ...
      if ($action === 'dismiss_guard_report') {
        ...
        if ($offenseType) {
          ...
          if ($level === 'MAJOR') {
              ...
              $cycleInfo = getStudentActiveMinorCycle((string)$report['student_id']);
...
$nteRows = db_all("
    SELECT nte.*, COALESCE(nte.case_id, uco.case_id) AS resolved_case_id 
...
foreach ($nteRows as $nte) {
    ...
    if ($cid > 0) $nteMap['case_' . $cid] = $nte;
    if (!empty($nte['offense_id'])) $nteMap['offense_' . $nte['offense_id']] = $nte;
    if (empty($nteMap['latest_student_' . $studentId])) {
        $nteMap['latest_student_' . $studentId] = $nte;
    }
}
                    ...
                    <div class="off-card major escalation-card" data-level="major">
                      ...
                      <?php elseif (in_array($caseStatus, ['RESOLVED', 'CLOSED', 'UNDER_APPEAL', 'CANCELLED', 'VOID'], true)): ?>
                        <?php 
                          ...
                          $decisionText = $groupData['case_decision'] ?: ($catTitle ? "Panel consensus adopted for {$catTitle}." : 'Panel decision finalized.');
                        ?>
                        <div style="margin-top:16px; margin-bottom:16px; padding:12px; background:#f0fdf4; border:1px solid #bbf7d0; border-radius:8px;">
                          <div style="font-size:11px; font-weight:800; color:#15803d; text-transform:uppercase; letter-spacing:0.5px; margin-bottom:8px;">
                            ...
                            UPCC Final Decision <?= $catNum > 0 ? "(Category {$catNum})" : "" ?>
                          </div>
